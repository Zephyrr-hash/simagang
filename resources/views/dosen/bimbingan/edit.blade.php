...
<nav aria-label="breadcrumb"><ol class="breadcrumb mb-0">
    ...
    <li class="breadcrumb-item"><a href="{{ route('dospem.index') }}">Bimbingan</a></li>
    <li class="breadcrumb-item active">{{ $mhs->nama_mhs }}</li>
</ol></nav>
@endsection
@push('styles')
<style>
.detail-grid{display:grid;grid-template-columns:300px 1fr;gap:1.25rem;max-width:1000px;}
...
.bim-item{background:#F8F7FF;border:1px solid #E0E7FF;border-radius:10px;padding:1rem 1.25rem;margin-bottom:0.875rem;}
.bim-date{font-size:0.78rem;font-weight:600;color:#4F46E5;margin-bottom:0.4rem;}
.bim-catatan{font-size:0.875rem;color:#374151;line-height:1.6;margin-bottom:0.75rem;}
.bim-file{font-size:0.8rem;color:#4F46E5;margin-bottom:0.75rem;}
.feedback-form{background:#ECFDF5;border:1px solid #A7F3D0;border-radius:9px;padding:1rem;}
.feedback-form label{display:block;font-size:0.78rem;font-weight:600;color:#065F46;margin-bottom:0.4rem;}
.feedback-form textarea{width:100%;padding:0.6rem 0.9rem;border:1.5px solid #A7F3D0;border-radius:8px;font-size:0.875rem;font-family:'Inter',sans-serif;color:#1E1B4B;background:#fff;outline:none;resize:vertical;transition:border-color 0.2s;}
.feedback-form textarea:focus{border-color:#059669;box-shadow:0 0 0 3px rgba(5,150,105,0.1);}
.btn-feedback{background:linear-gradient(135deg,#059669,#10B981);color:#fff;border:none;border-radius:8px;padding:0.55rem 1.25rem;font-weight:600;font-size:0.8rem;cursor:pointer;transition:opacity 0.2s;font-family:'Inter',sans-serif;margin-top:0.75rem;}
.btn-feedback:hover{opacity:0.88;}
.existing-feedback{background:#ECFDF5;border:1px solid #A7F3D0;border-radius:8px;padding:0.75rem 1rem;font-size:0.85rem;color:#065F46;margin-top:0.5rem;}
.existing-feedback strong{display:block;font-size:0.75rem;margin-bottom:0.25rem;}
...
<div class="detail-grid">
    ...
    {{-- Daftar Bimbingan --}}
    <div class="card">
        <h2 class="card-title">Riwayat Bimbingan ({{ $data->count() }} entri)</h2>
        @forelse($data as $item)
        <div class="bim-item">
            <p class="bim-date">📅 {{ \Carbon\Carbon::parse($item->tgl_bimbingan)->translatedFormat('d F Y') }}</p>
            <p class="bim-catatan">{{ $item->catatan }}</p>
            @if($item->file)
                <p class="bim-file"><a href="{{ asset('file/'.$item->file) }}" target="_blank">📎 Lihat File Laporan</a></p>
            @endif
            ...
            @else
                <div class="feedback-form">
                    <form action="{{ route('dospem.feedback', $item->bim_id) }}" method="POST">
                        @csrf
                        <input type="hidden" name="mhs_id" value="{{ $mhs->id }}">
                        <label>Berikan Feedback:</label>
                        <textarea name="feedback" rows="3" placeholder="Tulis feedback, saran, atau arahan..."></textarea>
        ...
        @empty
        <div class="empty-state"><p>Belum ada laporan bimbingan dari mahasiswa ini.</p></div>
        @endforelse
    </div>
</div>
@endsection
