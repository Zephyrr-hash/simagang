    ...
    <div class="form-card-body">
        ...
        <form action="{{ route('profile.update', 1) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="nama_depart" class="form-control-c" value="{{ old('nama_depart', $profile?->nama_depart) }}" required>
            </div>
            <div class="form-group">
                <label class="form-label">Alamat <span class="req">*</span></label>
                <textarea name="alamat_depart" class="form-control-c" rows="3" required>{{ old('alamat_depart', $profile?->alamat_depart) }}</textarea>
            </div>
            <div class="form-group">
                <label class="form-label">Telepon <span class="req">*</span></label>
                <input type="text" name="telepon_depart" class="form-control-c" value="{{ old('telepon_depart', $profile?->telepon_depart) }}" required>
            </div>
            <div class="form-group">
                <label class="form-label">NIDN <span class="req">*</span></label>
                <input type="text" name="NIDN" class="form-control-c" value="{{ old('NIDN', $profile?->NIDN) }}" required>
            </div>
            <div class="form-group">
                <label class="form-label">Foto Profil</label>
                @if($profile?->foto_depart)<p style="font-size:0.8rem;color:#6B7280;margin-bottom:0.4rem;">Foto saat ini: {{ $profile->foto_depart }}</p>@endif
                <input type="file" name="foto_depart" class="form-control-c" accept="image/jpeg,image/png,image/jpg">
            </div>
