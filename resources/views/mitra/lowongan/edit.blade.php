    ...
    <div class="form-card-body">
        ...
        <form action="{{ route('lowongan.update', $lowongan->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="nama_low" class="form-control-c" value="{{ old('nama_low', $lowongan->nama_low) }}" required>
            </div>
            <div class="form-group">
                <label class="form-label">Deskripsi <span class="req">*</span></label>
                <textarea name="deskripsi_low" class="form-control-c" rows="5" required>{{ old('deskripsi_low', $lowongan->deskripsi_low) }}</textarea>
            </div>
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select name="kategori_id" class="form-control-c" required>
                        ...
                        @foreach($kategori as $k)
                            <option value="{{ $k->id }}" {{ old('kategori_id', $lowongan->kategori_id) == $k->id ? 'selected' : '' }}>{{ $k->kategori }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label class="form-label">Telepon <span class="req">*</span></label>
                    <input type="text" name="telepon_low" class="form-control-c" value="{{ old('telepon_low', $lowongan->telepon_low) }}" required>
                </div>
            </div>
            <div class="form-row">
                <div class="form-group">
                    <label class="form-label">Kuota Mahasiswa <span class="req">*</span></label>
                    <input type="number" name="jumlah_mhs" class="form-control-c" value="{{ old('jumlah_mhs', $lowongan->jumlah_mhs) }}" min="0" required>
                </div>
                <div class="form-group">
                    <label class="form-label">Durasi (bulan) <span class="req">*</span></label>
                    <input type="number" name="durasi" class="form-control-c" value="{{ old('durasi', $lowongan->durasi) }}" min="1" required>
                </div>
            </div>
            <div class="form-group">
                <label class="form-label">Lokasi <span class="req">*</span></label>
                <input type="text" name="lokasi" class="form-control-c" value="{{ old('lokasi', $lowongan->lokasi) }}" required>
            </div>
            <div class="form-group">
                <label class="form-label">Foto Lowongan</label>
                @if($lowongan->foto_low)<p style="font-size:0.8rem;color:#6B7280;margin-bottom:0.4rem;">Foto saat ini: {{ $lowongan->foto_low }}</p>@endif
                <input type="file" name="foto_low" class="form-control-c" accept="image/jpeg,image/png,image/jpg">
                <p style="font-size:0.75rem;color:#9CA3AF;margin-top:0.3rem;">Kosongkan jika tidak ingin mengubah foto.</p>
            </div>
