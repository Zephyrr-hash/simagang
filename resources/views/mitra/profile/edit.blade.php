    ...
    <div class="form-card-body">
        ...
        <form action="{{ route('profile.update', 1) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="nama_mitra" class="form-control-c" value="{{ old('nama_mitra', $profile?->nama_mitra) }}" required>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" name="telepon_mitra" class="form-control-c" value="{{ old('telepon_mitra', $profile?->telepon_mitra) }}" required>
            </div>
            <div class="form-group">
                <label class="form-label">Fax</label>
                <input type="text" name="fax_mitra" class="form-control-c" value="{{ old('fax_mitra', $profile?->fax_mitra) }}">
            </div>
            <div class="form-group">
                <label class="form-label">Kabupaten/Kota <span class="req">*</span></label>
                <select name="kab_id" class="form-control-c" required>
                    <option value="">-- Pilih Kabupaten/Kota --</option>
                    @foreach($kabupatens as $kab)
                        <option value="{{ $kab->id }}" {{ old('kab_id', $profile?->kab_id) == $kab->id ? 'selected' : '' }}>{{ $kab->nama }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                <label class="form-label">Foto Profil</label>
                @if($profile?->foto_mitra)<p style="font-size:0.8rem;color:#6B7280;margin-bottom:0.4rem;">Foto saat ini: {{ $profile->foto_mitra }}</p>@endif
                <input type="file" name="foto_mitra" class="form-control-c" accept="image/jpeg,image/png,image/jpg">
            </div>
