...
<div class="form-card">
    <div class="form-card-header">
        ...
        <p>Buat project untuk mengorganisir pekerjaan mahasiswa magang.</p>
    </div>
    <div class="form-card-body">
        ...
        <form action="{{ route('spv.project.store') }}" method="POST">
            @csrf
            ...
            <div class="form-group">
                ...
                <select name="magang_id" class="form-control-c" required>
                    ...
                    @foreach($magangs as $mag)
                        <option value="{{ $mag->id }}"
                            {{ (old('magang_id', $selectedMagangId) == $mag->id) ? 'selected' : '' }}>
                            {{ $mag->mahasiswa?->nama_mhs ?? '—' }}
                            — {{ $mag->lowongan?->nama_low ?? '—' }}
                        </option>
                    @endforeach
                </select>
                @if($magangs->isEmpty())
                    <p style="font-size:0.78rem;color:#D97706;margin-top:0.35rem;">Belum ada mahasiswa aktif yang Anda supervisi.</p>
                @endif
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" name="nama_project" class="form-control-c"
                       ...
                       placeholder="Contoh: Pengembangan Dashboard Admin" required>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="deskripsi" class="form-control-c" rows="3"
                          placeholder="Jelaskan singkat tentang project ini...">{{ old('deskripsi') }}</textarea>
            </div>

            {{-- Tujuan --}}
            <div class="form-group">
                <label class="form-label">Tujuan / Scope</label>
                <textarea name="tujuan" class="form-control-c" rows="3"
                          placeholder="Apa yang ingin dicapai dari project ini?">{{ old('tujuan') }}</textarea>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" name="teknologi" class="form-control-c"
                       value="{{ old('teknologi') }}"
                       placeholder="Contoh: Laravel, Vue.js, MySQL, Figma">
                <p style="font-size:0.75rem;color:#9CA3AF;margin-top:0.3rem;">Pisahkan dengan koma jika lebih dari satu.</p>
            </div>
            ...
            <div class="form-actions">
                ...
                <a href="{{ route('spv.project.index') }}" class="btn-cancel">Batal</a>
            </div>
        </form>
    </div>
</div>
