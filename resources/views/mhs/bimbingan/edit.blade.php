    ...
    <div class="form-card-body">
        ...
        <form action="{{ route('bimbingan.update', $bimbingan->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @if($bimbingan->file)<p style="font-size:0.8rem;color:#6B7280;margin-bottom:0.4rem;">File saat ini: <a href="{{ asset('file/'.$bimbingan->file) }}" target="_blank" style="color:#4F46E5;">{{ $bimbingan->file }}</a></p>@endif
                <input type="file" name="file" class="form-control-c" accept=".pdf,.doc,.docx">
                <p style="font-size:0.75rem;color:#9CA3AF;margin-top:0.3rem;">Kosongkan jika tidak ingin mengubah file.</p>
            </div>
